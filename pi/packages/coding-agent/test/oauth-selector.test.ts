...
describe("OAuthSelectorComponent", () => {
	...
	it("keeps built-in API key providers separate from OAuth-only providers", () => {
		...
		expect(getApiKeyProviderDisplayName("amazon-bedrock")).toBe("Amazon Bedrock");
	...
	it("uses registry auth status for provider-configured API keys", () => {
		const authStorage = AuthStorage.inMemory();
		const selector = new OAuthSelectorComponent(
			"login",
			authStorage,
			[{ id: "custom-provider", name: "Custom Provider", authType: "api_key" }],
			() => {},
			() => {},
			(providerId) => {
				expect(providerId).toBe("custom-provider");
		...
		expect(output).toContain("unconfigured");
		expect(output).toContain("custom API key");
	});
});
