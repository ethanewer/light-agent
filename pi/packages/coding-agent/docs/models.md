...
| `supportsLongCacheRetention` | Whether the provider accepts Anthropic long cache retention (`cache_control.ttl: "1h"`) when cache retention is `long`. Defaults to `true` only for known official Anthropic API URLs; set this explicitly for proxies or compatible providers. |
