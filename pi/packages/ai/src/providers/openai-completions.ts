...
function buildParams(
	...
	const supportsOpenAIPromptCacheRetention =
	...
	const params: OpenAI.Chat.Completions.ChatCompletionCreateParamsStreaming = {
		...
		prompt_cache_key:
			...
			(cacheRetention === "long" && supportsOpenAIPromptCacheRetention)
				? options?.sessionId
				: undefined,
		prompt_cache_retention: cacheRetention === "long" && supportsOpenAIPromptCacheRetention ? "24h" : undefined,
	};
...
function supportsLongCacheRetentionByDefault(
	model: Model<"openai-completions">,
	cacheControlFormat: OpenAICompletionsCompat["cacheControlFormat"],
): boolean {
	...
	return model.baseUrl.includes("api.openai.com");
}
...
export function convertMessages(
	...
	for (let i = 0; i < transformedMessages.length; i++) {
		...
		if (msg.role === "user") {
			...
			// Some providers don't accept null assistant content, use an empty
			// string instead. DeepSeek's thinking + tool replay requires
			// reasoning_content alongside an empty content field.
			const assistantMsg: ChatCompletionAssistantMessageParam = {
				role: "assistant",
				content:
					compat.requiresAssistantAfterToolResult || compat.requiresReasoningContentOnAssistantMessages
						? ""
						: null,
			};
...
function detectCompat(model: Model<"openai-completions">): ResolvedOpenAICompletionsCompat {
	...
	return {
		...
		maxTokensField: isDeepSeek || useMaxTokens ? "max_tokens" : "max_completion_tokens",
		...
		supportsLongCacheRetention: supportsLongCacheRetentionByDefault(model, cacheControlFormat),
	};
}
