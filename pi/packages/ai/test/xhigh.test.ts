...
describe.skipIf(!process.env.OPENAI_API_KEY)("xhigh reasoning", () => {
	describe("codex-max (supports xhigh)", () => {
		...
		it("should work with openai-responses", { retry: 2, timeout: 60000 }, async () => {
	...
	describe("gpt-5-mini (does not support xhigh)", () => {
		it("should error with openai-responses when using xhigh", { retry: 2, timeout: 60000 }, async () => {
		...
		it("should error with openai-completions when using xhigh", { retry: 2, timeout: 60000 }, async () => {
