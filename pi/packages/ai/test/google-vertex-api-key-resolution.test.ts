...
describe("google-vertex api key resolution", () => {
	...
	it("does not append apiVersion when custom baseUrl includes alpha or beta version", async () => {
		const customModel: Model<"google-vertex"> = {
			...model,
			baseUrl: "https://proxy.example.com/v1alpha/projects/test-project/locations/global",
		};
		const stream = streamGoogleVertex(customModel, context, {
			project: "test-project",
			location: "us-central1",
